class DOMUtil {
  ...
  static observeCssClassChanges(callback, element = document.body) {
    let observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === "attributes") {
          if (callback) callback(mutation.target.classList);
        }
      });
    });
  ...
  static cleanupObserver(observer) {
    if (observer) {
      observer.disconnect();
    }
  }
  ...
  static recurseDisableElements(elem, disabledElements) {
    if (elem) {
      ...
      if (elem.tagName && disabledElements.indexOf(elem.tagName.toLowerCase()) != -1) {
