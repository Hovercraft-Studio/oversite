...
public class DashboardPoster : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("Starting DashboardPoster");
        InvokeRepeating("PostHeartbeat", 10, interval);
    ...
    void OnDestroy()
    {
        CancelInvoke(nameof(PostHeartbeat));
    }
    ...
    {
        if (showDebugLogs)
            Debug.Log("[DashboardPoster] " + message);
    }
    ...
    {
        InvokeRepeating(nameof(CollectScreenshot), 5, interval);
    }
